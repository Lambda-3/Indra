...
public class ANNIndex implements AnnoyIndex {
    ...
    public void getNodeVector(final long nodeOffset, double[] v) {
    ...
    @Override
    @SuppressWarnings("deprecation")
    public void getItemVector(int itemIndex, double[] v) {
    ...
    public final double[] getItemVector(final int itemIndex) {
        return getNodeVector(itemIndex * NODE_SIZE);
    }

    public double[] getNodeVector(final long nodeOffset) {
        double[] v = new double[DIMENSION];
        getNodeVector(nodeOffset, v);
        return v;
    }

    private static float norm(final double[] u) {
        float n = 0;
        for (double x : u)
            n += x * x;
    ...
    private static float euclideanDistance(final double[] u, final double[] v) {
    ...
    public static float cosineMargin(final double[] u, final double[] v) {
        double d = 0;
        for (int i = 0; i < u.length; i++)
            d += u[i] * v[i];
    ...
    public static float euclideanMargin(final double[] u,
                                        final double[] v,
                                        final float bias) {
        float d = bias;
        for (int i = 0; i < u.length; i++)
    ...
    private static boolean isZeroVec(double[] v) {
    ...
    @Override
    public final List<Integer> getNearest(final double[] queryVector,
                                          final int nResults) {
        ...
        while (nearestNeighbors.size() < roots.size() * nResults && !pq.isEmpty()) {
            ...
            double[] v = getNodeVector(topNodeOffset);
    ...
    public static void main(final String[] args) throws IOException {
        ...
        // input vector
        double[] u = annIndex.getItemVector(queryItem);
        ...
        for (int nn : nearestNeighbors) {
            double[] v = annIndex.getItemVector(nn);
